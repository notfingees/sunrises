...
class InterestsSettingsDetailSearchViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, UISearchControllerDelegate, NSFetchedResultsControllerDelegate, UISearchResultsUpdating {
   ...
   // @IBOutlet weak var doneLabel: UILabel!
    ...
    override func dismiss(animated flag: Bool, completion: (() -> Void)? = nil) {
        if isFiltering{
            ...
            NotificationCenter.default.post(name: Notification.Name.isvcSettingsBack, object: nil)
    ...
    override func viewDidLoad() {
        ...
        print("in interests settings detail search view controller, category is \(category)")
       ...
       // (sbTextField!.value(forKey: "placeholderLabel") as? UILabel)!.textColor = UIColor.white
        
        //let attributeDict = [NSAttributedString.Key.foregroundColor: UIColor.white]
        //sbTextField!.attributedPlaceholder = NSAttributedString(string: "Search", attributes: attributeDict)
    ...
    func getAllInterests() -> [NSManagedObject]{
        ...
        let fetchRequest2 = NSFetchRequest<NSManagedObject>(entityName: "Interest")
        fetchRequest2.predicate = NSPredicate(format: "category == %@ AND recommended == true AND user_added == false", category)
        do {
            _recommendedInterests = try managedContext.fetch(fetchRequest2)
        } catch let error as NSError {
            print("Could not fetch. \(error), \(error.userInfo)")
                    ...
                    if error != nil {
                       ...
                       // let persons = NSMutableArray()
                        ...
                        for i in 0 ..< jsonResult.count{
                         ...
                         //       print("setting \(indexes_temp[0]) to recommended = true in InterestsSettingsdetailSearchViewcontroller")
